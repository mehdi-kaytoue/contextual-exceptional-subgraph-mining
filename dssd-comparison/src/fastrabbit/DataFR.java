...
public class DataFR {
    ...
    int bsInitPattern(BitSet p, MutableInteger depth) {//, MutableInteger idNum) {
	...
	for(int i = 0; i < nbAttributes; i++) {
		...
		if(s.charAt(0) == '[') {
		    ...
		    //expectedOnes = expectedOnes + 2;//b - a + 3;
	...
	*/
     ...
     */
    public Tuple createTuple(String s) {
	Tuple tup = new Tuple();
	BitSet bits = new BitSet();
	String parts[] = s.split(";");
	int offset = 0;
	...
	*/
	...
	for(int i = 0; i < nbAttributes; i++){
		...
		if(attributeTypes.elementAt(i) == 1) {
		    ...
		    int l = tmp.cardinality();
		    Integer last = new Integer((int) first + l-2);
		    result+="[" + findKey(modalityNames.get(i), modalityNum.get(indexNum).get(first))+ "," + 
				     findKey(modalityNames.get(i), modalityNum.get(indexNum).get(last)) + "];";
		    indexNum++;
		}
	    }
