...
public class Enumeration {
	...
	*/
    ...
    public void enumerate(int depth, int offset, int idNum, int expectedOnes, Check c, int first){
	...
	if(depth == data.nbAttributes) {
		...
		for(int i = 0; i < gr.cc.size(); i++) {
			...
			c.computeSkyline(gr.cc.elementAt(i), skyline, pattern, skylineMotif);
		}
		//print Pattern
		...
		if(data.attributeTypes.elementAt(depth) == 1) {
			...
			while (it.hasNext()) {
				...
				if(allGraphs.elementAt(depth + 1).nbCCValides > 0) 
				    ...
				    enumerate(depth + 1, data.nbBitsOpt.elementAt(depth + 1), idNum, expectedOnes + 1, c, first);
			    }
			    if(!v.getKey().equals(0))
				expectedOnes = expectedOnes - 1;
			}
		...
		//Numerical attribute
			...
			if( (r != 2) && (allGraphs.elementAt(depth + 1).nbCCValides > 0)) {
			    ...
			    enumerate(depth + 1, data.nbBitsOpt.elementAt(depth + 1), idNum + 1, expectedOnes + 2, c, first);
			}
	...
	for(int i = 0; i < d.size(); i++) {
	    ...
	    //data.printPattern(m.elementAt(i));
	    //s.printCC(data);
	    
	    if(FastRabbit.prettyFileOutput) s.writeCC(data.toStringPattern(m.elementAt(i))); // write the pattern in a file for vizu
