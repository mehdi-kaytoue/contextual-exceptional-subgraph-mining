...
(function(window) {
    ...
    function launchThirdPart(){
        $(function() {



            /* Reading pattern from ESCARGOT's output */
            // var arrayId = [];
            // var arrayContext = [];
            //  var arrayPattern = [];
            var layerNames = {};

            //$.getJSON("/uploads/a_labels.json", function (json) {
            //
            //    layerNames.npc_dota_roshan_spawner= 'Roshan';
            //    layerNames.dota_item_rune_spawner= 'Runes';
            //    layerNames.ent_dota_tree='Trees';
            //    layerNames.npc_dota_fort= 'Ancients';
            //    layerNames.ent_dota_shop= 'Shops';
            //    layerNames.npc_dota_tower= 'Towers';
            //    layerNames.npc_dota_barracks= 'Barracks';
            //    layerNames.npc_dota_building= 'Buildings';
            //    layerNames.trigger_multiple= 'Neutral Camps Spawn Boxes';
            //    layerNames.npc_dota_neutral_spawner= 'Neutral Camps';
            //
            //    $.each(json.patterns, function (i, fb) {
            //        //arrayId.push(fb.id);
            //        layerNames["pattern"+fb.id]= ""+fb.id+":" +fb.context;
            //        //arrayContext.push(fb.context);
            //        //arrayPattern.push(fb.pattern);
            //    });
            //    //console.log(arrayId.length + ' patterns have been read.');
            //
            //});
            //log(layerNames);
            /* End pattern from ESCARGOT's output */
            ...
            var wms = new OpenLayers.Layer.TMS("Dota 2 Map", "tiles/", {
                type: 'png',
                getURL: get_my_url
            });
            map.addLayer(wms);
            ...
            var size = new OpenLayers.Size(21, 25);
            var offset = new OpenLayers.Pixel(-(size.w / 2), -size.h);
            var icon = new OpenLayers.Icon('/img/marker1.png', size, offset);
            ...
            data.links.forEach(function(d) {
                ...
                var target = data.nodes[d.target];


                var start_x = parseInt(source.name.split('/')[0]);
                var start_y = (- 1 * parseInt(source.name.split('/')[1]) + 800);

                start_x = ((start_x + 8200)/(8200+ 8200)*5120);
                start_y = ((start_x + 8842)/(8842+ 8200)*5120);



                var end_x = parseInt(target.name.split('/')[0]);
                var end_y = (- 1 * parseInt(target.name.split('/')[1])+ 800);


                end_x = ((end_x + 8200)/(8200+ 8200)*5120);
                end_y = ((end_y + 8842)/(8842+ 8200)*5120);




                start_geo = geoPointToLatLng(start_x, 5120 - start_y)
                end_geo = geoPointToLatLng(end_x,  5120- end_y)

                var start_point = new OpenLayers.Geometry.Point(start_geo.lat, start_geo.lon);
                var end_point = new OpenLayers.Geometry.Point(end_geo.lat, end_geo.lon);
                ...
                style.strokeColor = getRandomColor();

                var vector = new OpenLayers.Layer.Vector(d.name, {styleMap: style});
            ...
            //$.getJSON("uploads/a_data.json", function(data) {
            //    console.log('okkkk')
            //   // console.log(data);
            //    var markers = {}
            //    for (var k in data) {
